...
* [RaceChrono](https://racechrono.com/) CSV logs
...
```bash
python3 motec_log_generator.py /path/to/my/data/racechrono_data.csv RACECHRONO
```

This will generate a motec .ld file `/path/to/my/data/racechrono_data.ld`.

RaceChrono CSV files are parsed looking for the 'Time (s)' header row and mapped to standard MoTeC channel names and units automatically where applicable.
...
usage: motec_log_generator.py [-h] [--output OUTPUT] [--frequency FREQUENCY]
                              ...
                              log {CAN,CSV,ACCESSPORT,RACECHRONO}

Generates MoTeC .ld files from external log files generated by: CAN bus dumps,
CSV files, COBB Accessport CSV files, or RaceChrono CSV files

positional arguments:
  log                   Path to logfile
  {CAN,CSV,ACCESSPORT,RACECHRONO}
                        Type of log to process
...
and units will be directly copied over. RaceChrono CSV files are parsed
looking for the 'Time (s)' header row and mapped to standard MoTeC channel
names and units automatically where applicable.
```
