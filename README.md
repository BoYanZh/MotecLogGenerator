...
A high-performance Python utility for generating MoTeC `.ld` / `.ldx` log files from external telemetry sources (RaceChrono, AiM Solo / RaceStudio, Racelogic VBOX, COBB Accessport, PB Buddy, CAN bus logs, and generic CSVs). 
...
### 3. Automatic Log Type Detection (`AUTO`)
```bash
# Auto-detect log format (PB Buddy, AiM, RaceChrono, Accessport, etc.)
python motec_log_generator.py /path/to/session_export.csv AUTO
```

### 4. CAN Bus & Accessport Logs
```bash
...
usage: motec_log_generator.py [-h] [--output OUTPUT] [--g-source {auto,sensor,calc}]
                              ...
                              log {CAN,CSV,ACCESSPORT,RACECHRONO,RCZ,PBBUDDY,AUTO}
